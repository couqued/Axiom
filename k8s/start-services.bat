@echo off
REM K8s 서비스 기동 스크립트 (장 시작 전 배치를 위해 기동)
...
if %errorlevel% neq 0 ( echo [ERROR] Zookeeper 기동 실패 & exit /b 1 )
kubectl wait --for=condition=ready pod -l app=postgres -n axiom --timeout=120s
if %errorlevel% neq 0 ( echo [ERROR] PostgreSQL 기동 실패 & exit /b 1 )
...
if %errorlevel% neq 0 ( echo [ERROR] Kafka 기동 실패 & exit /b 1 )
